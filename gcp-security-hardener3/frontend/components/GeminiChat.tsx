...
import { chatWithGemini, analyzeScanWithGemini, ChatMessage } from '@/lib/api';
...
export default function GeminiChat({ scanResults, context = null, className = '' }: GeminiChatProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (scanResults && context === 'scan_results' && userMessage.toLowerCase().includes('analyze')) {
        ...
        const response = await analyzeScanWithGemini(scanResults);
    ...
    return (
      <div className={`fixed bottom-4 right-4 z-50 ${className}`}>
        <button
          ...
          className="bg-primary-600 text-white rounded-full p-4 shadow-lg hover:bg-primary-700 transition-colors flex items-center gap-2"
        >
    ...
    <div className={`bg-white border border-gray-200 rounded-lg shadow-lg flex flex-col ${className}`} style={{ height: '600px' }}>
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gradient-to-r from-primary-50 to-blue-50">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-semibold text-gray-900">AI Security Assistant</h3>
      ...
      <div className="p-4 border-t border-gray-200">
        <div className="flex gap-2">
          <textarea
            ...
            className="flex-1 p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none"
            rows={2}
            disabled={isLoading}
          />
          <button
